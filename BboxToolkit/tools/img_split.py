...
def crop_and_save_img(info, windows, window_anns, img_dir, no_padding,
    ...
    # 缩放逻辑：当宽度大于1024且高度小于1024时，进行等比例缩放
...
def single_split(arguments, sizes, gaps, img_rate_thr, iof_thr, no_padding,
    ...
    # Instead of asserting, we check and log the issue, then skip the image
    if not patch_infos and not (filter_empty and info['ann']['bboxes'].size == 0):
        logger.error(f"Skipping image due to no patches generated and conditions not met. Filename: {info['filename']}, BBox Size: {info['ann']['bboxes'].size}")
        return []  # Return an empty list or similar structure that aligns with your data handling
